...
**Theorem**: 
A statistic $T(X)$ is sufficient for $\theta$ if and only if the joint density (or mass) function $f(x|\theta)$ can be factored into the form
...
**Implications**:
The Factorization Theorem is useful for identifying sufficient statistics, which summarize all necessary information from a sample about the parameter $\theta$.
